...
const Player = ({ player }) => {
  ...
  const followHandler = () => {
    if (isUser) {
      updateData(
        `/user/player/follow?__p=${player._id}`,
        { firstName, lastName },
        __u__.token
      ).then((res) => {
        if (res.success) {
  ...
  const UnfollowHandler = () => {
    updateData(`/user/player/unfollow?__p=${player._id}`, {}, __u__.token).then(
      (res) => {
        if (res.success) {
    ...
    <div className={style.player}>
      <div className={style.playerImage}>
        <Image
          height="948"
          width="948"
          src={imageResize(player.image, "c_thumb,g_face,h_948,w_948")}
          alt=""
        />
      </div>
      <div className={style.playerContent}>
        ...
        <h6>
          ...
          <Image
            height="32"
            width="32"
            src="/images/icon/player-icon.png"
            alt=""
          />
          {player.league}
        </h6>
        <p>{player.description}</p>
        {isUser && info?._players.includes(player._id) ? (
